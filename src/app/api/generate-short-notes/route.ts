...
export async function POST(request: NextRequest) {
  try {
    ...
    const coinResult = await checkAndDeductCoins(user.id, "SHORT_NOTES");
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/generate-short-notes`, {
        method: "POST",
        headers: {
      ...
      return NextResponse.json({ error: "Failed to generate short notes. Your coin has been refunded." }, { status: 500 });
    }
  } catch (error) {
