...
export async function POST(request: NextRequest) {
  try {
    ...
    const coinResult = await checkAndDeductCoins(user.id, "MCQ");
    if (!coinResult.success) {
      return NextResponse.json({ error: coinResult.message }, { status: 402 }); // Payment Required
    }
    ...
    try {
      ...
      return NextResponse.json({ error: "Failed to generate MCQs. Your coin has been refunded." }, { status: 500 });
    }
  } catch (error) {
